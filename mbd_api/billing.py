...
def is_subscription_active_status(status: str | None) -> bool:
...
def normalize_subscription_snapshot(
    ...
    *,
    ...
    last_checkout_completed_at: str | None = None,
    ...
    product_id = _coerce_text(product.get("id")) if isinstance(product, dict) else _coerce_text(product)
...
def _extract_event_identifiers(event: dict[str, Any]) -> dict[str, str | None]:
    event_type = _coerce_text(event.get("type"))
    try:
...
def _to_plain_dict(value: Any) -> Any:
    ...
    if hasattr(value, "__dict__"):
        public_fields = {str(k): _to_plain_dict(v) for k, v in vars(value).items() if not str(k).startswith("_")}
        if public_fields:
...
def _to_json_safe(value: Any) -> Any:
    ...
    if isinstance(normalized, datetime):
        return (
            normalized.astimezone(UTC).isoformat() if normalized.tzinfo else normalized.replace(tzinfo=UTC).isoformat()
        )
...
def subscription_allows_api_access(
    *,
    ...
    if period_end_dt is None or period_end_dt <= reference:
        return False
    return ended_at_dt is None or ended_at_dt > reference


def _subscription_items_period_timestamp(subscription: dict[str, Any], field_name: str, prefer: str) -> str | None:
    items = subscription.get("items") or {}
    item_rows = items.get("data") if isinstance(items, dict) else None
...
def _first_present_timestamp(*values: Any) -> str | None:
    for value in values:
...
def _snapshot_field(snapshot: dict[str, Any] | None, field_name: str) -> Any:
