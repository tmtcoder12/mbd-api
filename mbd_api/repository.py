...
class SupabaseStore:
    ...
    def close(self) -> None:
        ...
        self._close_session_quietly(self.session)

    def healthcheck(self, timeout_s: float | None = None) -> bool:
        """Perform a lightweight authenticated PostgREST readiness check."""
        data = self._request(
            "GET",
            "/rest/v1/restaurants",
            query={"select": "id", "limit": "1"},
            timeout_s=max(0.25, float(timeout_s)) if timeout_s is not None else None,
        )
    ...
    def upsert_restaurant(self, restaurant_id: str, slug: str | None = None, name: str | None = None) -> str:
    ...
    def get_restaurant_subscription_by_subscription_id(self, stripe_subscription_id: str) -> dict[str, Any] | None:
    ...
    def get_restaurant_subscription_by_customer_id(self, stripe_customer_id: str) -> dict[str, Any] | None:
    ...
    def upsert_restaurant_subscription(self, restaurant_id: str, fields: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = {
            ...
            "last_synced_at": fields.get("last_synced_at") or datetime.now(UTC).isoformat(),
            "updated_at": fields.get("updated_at") or datetime.now(UTC).isoformat(),
        }
    ...
    def get_stripe_webhook_event(self, event_id: str) -> dict[str, Any] | None:
    ...
    def get_restaurant_system_prompt(self, restaurant_id: str) -> str | None:
        rid = self._require_uuid(restaurant_id, "restaurant_id")
        data = self._request(
            "GET",
            "/rest/v1/restaurants",
    ...
    def begin_ingest_run(self, restaurant_id: str, model: str, source_name: str, total_chunks: int) -> str | None:
        rid = self._require_uuid(restaurant_id, "restaurant_id")
        payload = {
            "restaurant_id": rid,
            "model": model,
    ...
    def get_knowledge_chunk_fingerprints(self, restaurant_id: str) -> list[dict[str, Any]]:
        """Return lightweight identifiers used to skip unchanged embeddings."""
        rid = self._require_uuid(restaurant_id, "restaurant_id")
        data = self._request(
            "GET",
            "/rest/v1/knowledge_chunks",
            query={
                "select": "source_key,external_id,content_hash,embedding_model",
                "restaurant_id": f"eq.{rid}",
                "limit": "10000",
            },
        )
        if not isinstance(data, list):
            raise SupabaseStoreError("knowledge chunk fingerprint lookup returned non-list payload")
        return [row for row in data if isinstance(row, dict)]

    def stage_knowledge_chunks(self, run_id: str, rows: list[dict[str, Any]]) -> None:
        """Upload a batch without changing the active knowledge set."""
        run_uuid = self._require_uuid(run_id, "run_id")
        ...
        self._request(
            "POST",
            "/rest/v1/knowledge_chunk_staging",
            payload=payload,
            query={"on_conflict": "run_id,source_key,external_id"},
    ...
    def finalize_ingest_run(self, run_id: str, prune: bool = False) -> dict[str, int]:
        ...
        data = self._request(
            "POST",
            "/rest/v1/rpc/finalize_ingest_run",
            payload={"p_run_id": run_uuid, "p_prune": bool(prune)},
        )
        row: Any = data[0] if isinstance(data, list) and data else data
        if not isinstance(row, dict):
            raise SupabaseStoreError("finalize_ingest_run returned an invalid payload")
        return {
            "activated_chunks": int(row.get("activated_chunks") or 0),
            "pruned_chunks": int(row.get("pruned_chunks") or 0),
        }
